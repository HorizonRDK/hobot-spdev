...
MIPI_SENSOR_INFO_S SENSOR_2LANE_SC031GS_10BIT_LINEAR_INFO =
{
	...
	.sensorInfo = {
		...
		.sensor_name = "sc031gs", // sensor的名字，在libcam.so中会根据这个名字组合出 libf37.so
...
// SC031GS
MIPI_ATTR_S MIPI_2LANE_SC031GS_10BIT_LINEAR_ATTR =
{
	.mipi_host_cfg = {
		...
		.mipiclk = 720, // sensor 输出总的 mipibit rate, 单位Mbits/秒
		.fps = F_FPS, // sensor 输出实际帧率, 和 MIPI_SENSOR_INFO_S 是什么关系？
		.width = F_WIDTH, // sensor实际输出的分辨率，一般根据sensor供应商的配置参数配置就行
		.height = F_HEIGHT,
		.linelenth = 640,//878, // sensor 输出带 blanking 的总行长， 一般根据sensor供应商的配置参数配置就行
		.framelenth = 480,//2732, // sensor 输出带 blanking的总行数
...
VIN_DEV_ATTR_S DEV_ATTR_2LANE_SC031GS_10BIT_LINEAR_BASE = {
	.stSize = { // 输入的数据
...
VIN_PIPE_ATTR_S PIPE_ATTR_2LANE_SC031GS_10BIT_LINEAR_BASE = {
	...
	.calib = { // 是否开启 sensor 矫正数据加载，1 是开启，0 是关闭。ISP使用这些数据
		...
		.lname = "libsc031gs_linear.so", // 对应使用的校准库
	}
};

VIN_DIS_ATTR_S DIS_ATTR_SC031GS_BASE = {
  .picSize = {
    .pic_w = F_WIDTH-1,
	.pic_h = F_HEIGHT-1,
  },
...
VIN_LDC_ATTR_S LDC_ATTR_SC031GS_BASE = {
    .ldcEnable = 0,
    .ldcPath = {
        .rg_y_only = 0,
        .rg_uv_mode = 0,
