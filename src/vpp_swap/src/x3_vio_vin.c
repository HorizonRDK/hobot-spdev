...
int x3_sensor_init(int devId, x3_vin_info_t *vin_info)
{
    ...
    LOGD_print("hb sensor init success...\n");
...
int x3_mipi_init(x3_vin_info_t *vin_info)
{
    ...
    LOGD_print("hb mipi init success...\n");
...
static int x3_mipi_deinit(int mipiIdx)
{
    ...
    LOGD_print("x3_sensor_deinit end==mipiIdx %d====\n", mipiIdx);
    ret = HB_MIPI_Clear(mipiIdx);
    if (ret < 0) {
        LOGE_print("HB_MIPI_Clear error!\n");
        return ret;
    }
    LOGD_print("x3_mipi_deinit end======\n");
    return ret;
}
...
int x3_vin_init(x3_vin_info_t *vin_info)
{
    ...
    LOGD_print("vin_info->mipi_attr.mipi_host_cfg.channel_num: %d\n", vin_info->mipi_attr.mipi_host_cfg.channel_num);
    ...
    LOGD_print("start HB_VIN_SetDevAttr \n");
    ...
    LOGD_print("HB_VIN_SetDevAttr ok!\n");

    if (vin_info->isp_enable == 1) {
        ret = HB_VIN_SetPipeAttr(pipeId, &vin_info->pipeinfo); // isp init
        if (ret < 0) {
            pr_err("HB_VIN_SetPipeAttr error!\n");
            goto pipe_err;
        }
        LOGD_print("HB_VIN_SetPipeAttr ok!\n");
    ...
    LOGD_print("HB_VIN_SetDevBindPipe ok!\n");
...
int x3_vin_sif_get_data(const int pipe_id, hb_vio_buffer_t *sif_raw, int timeout)
{
    ...
    ret = HB_VIN_GetDevFrame(pipe_id, 0, sif_raw, timeout);
    if (ret) {
        LOGE_print("HB_VIN_GetDevFrame error, ret=%d\n", ret);
    } else {
        /*x3_normal_buf_info_print(sif_raw);*/
...
int x3_vin_sif_release_data(const int pipe_id, hb_vio_buffer_t *sif_raw)
{
    ...
    ret = HB_VIN_ReleaseDevFrame(pipe_id, 0, sif_raw);
    if (ret < 0) {
        LOGE_print("HB_VIN_ReleaseDevFrame error!!!\n");
    }
...
int x3_vin_isp_get_data(const int pipe_id, hb_vio_buffer_t *isp_yuv, int timeout)
{
    ...
    ret = HB_VIN_GetChnFrame(pipe_id, 0, isp_yuv, timeout);
    if (ret < 0) {
        LOGE_print("HB_VIN_GetChnFrame error!!!\n");
    } else {
        /*x3_normal_buf_info_print(isp_yuv);*/
        size = isp_yuv->img_addr.stride_size * isp_yuv->img_addr.height;
        if (size == 0) // 有可能是因为模式不是offline的，所以获取不到raw图
...
int x3_vin_isp_release_data(const int pipe_id, hb_vio_buffer_t *isp_yuv)
{
    ...
    ret = HB_VIN_ReleaseChnFrame(pipe_id, 0, isp_yuv);
    if (ret < 0) {
        LOGE_print("HB_VIN_ReleaseChnFrame error!!!\n");
    }
